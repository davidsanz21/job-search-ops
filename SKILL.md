...
- **PDF rendering:** use `scripts/render_pdf.py` (run `python scripts/render_pdf.py --help` once per session). Styles: `cv` (2–3 pages max), `letter` (1 page), `cheatsheet` (dense prep sheet). The script reports the page count — if a CV exceeds 3 pages or a letter exceeds 1, fix the content by restructuring (reorder, consolidate sections) before compressing typography.
